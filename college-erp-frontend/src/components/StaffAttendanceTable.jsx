...
const StaffAttendanceTable = ({ onEdit }) => {
  ...
  const fetchAttendance = async () => {
    try {
      ...
      console.error(error);
      toast.error('Failed to load staff attendance', { id: 'staff-attendance-error' });
      setAttendance([]);
    } finally {
      setLoading(false);
    }
  ...
  const filteredData = Array.isArray(attendance) ? attendance.filter(record => {
    if (!record?.Staff) return false;
    const name = (record.Staff.fullName || '').toLowerCase();
    const reg = (record.Staff.staffId || '').toLowerCase();
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'Leave': return <span className="px-2.5 py-1 text-xs font-medium bg-indigo-50 text-indigo-600 rounded-lg">Leave</span>;
                ...
                <tr key={record.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className="font-semibold text-text-main">{record.Staff?.fullName}</span>
                      <span className="text-xs text-text-muted">{record.Staff?.staffId}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className="text-text-main">{record.Staff?.designation}</span>
                      <span className="text-xs text-text-muted">{record.Staff?.department}</span>
                    </div>
                  </td>
